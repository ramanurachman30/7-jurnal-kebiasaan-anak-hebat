...
let isAllowToPublish = $("meta[name='permission']").attr("publish");
let isAllowToPreview = $("meta[name='permission']").attr("preview");
let previewName = $("meta[name='permission']").attr("preview-name");
...
let param = $("meta[name='params']").attr("param");
        ...
        },
        {
          ...
          render: function (data, type, row) {
            var dom = `
                        ...
                        `;
            var getParam = "";
            if (param) getParam = "?q=" + param;
            if (isAllowToUpdate) {
              dom += `<div class="px-3 menu-item">
                                        <a href="${firstSegment}/${data.id}/edit${getParam}" class="px-3 menu-link" data-kt-user-table-filter="edit_row">
                                            Edit
                                        </a>
                                    </div>`;
            }
            ...
            // if (isAllowToPreview) {
            //   dom += `<div class="px-3 menu-item">
            //                         <a href="${hostUrl}/preview/${previewName}/${data.id}" class="px-3 menu-link" target="_blank">
            //                             Preview
            //                         </a>
            //                     </div>`;
            // }

            if (isAllowToPublish && data.status != "Published") {
              dom += `<div class="px-3 menu-item">
                                    <a data-remote="${firstSegment}/${data.id}/publish" class="px-3 menu-link" data-bs-toggle="modal"
                                    data-bs-target="#modal_publish">
                                        Publish
                                    </a>
                                </div>`;
            }
            ...
            if (isAllowToPublish && data.status == "Published") {
              dom += `<div class="px-3 menu-item">
                                    <a data-remote="${firstSegment}/${data.id}/draft" class="px-3 menu-link" data-bs-toggle="modal"
                                    data-bs-target="#modal_draft">
                                        Set As Draft
                                    </a>
                                </div>`;
            }
            ...
            $.ajax({
              ...
              success: function (resp) {
                Swal.fire({
                  ...
                  // Vision and Missions: Reload page to update Create button visibility
