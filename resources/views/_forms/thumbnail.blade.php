<div class="col-lg-9 col-xl-{{ isset($data['column']) ? $data['column'] : '9' }}">
    ...
    ?>
    <div class="image-input image-input-empty thumbnail-input" data-kt-image-input="true"
        style="background-image: url({{ $image }})" data-url="{{ url('upload_file') }}" data-bucket="avatar"
        data-path="avatar">
        <div class="image-input-wrapper w-125px h-125px"></div>
        <label class="bg-white shadow btn btn-icon btn-circle btn-active-color-primary w-25px h-25px"
            data-kt-image-input-action="change" data-bs-toggle="tooltip" data-bs-dismiss="click" title="Ganti Foto">
            <i class="bi bi-pencil-fill fs-7"></i>
            @if (isset($data['value']))
                <input class="thumbnail-image" type="hidden" name="{{ $data['name'] }}" id="{{ $data['name'] }}"
                    value="{{ $data['value'] }}" />
            @else
                <input class="thumbnail-image" type="hidden" name="{{ $data['name'] }}" id="{{ $data['name'] }}"
                    value="{{ old($data['name']) }}" />
