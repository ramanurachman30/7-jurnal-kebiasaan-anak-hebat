...
<div x-data="{ current: 0 }" class="w-full py-4 md:py-6">
    ...
    <div class="relative overflow-hidden w-full h-[400px] md:h-[650px] rounded-xl">
        <template x-for="(video, index) in {{ Js::from($videos) }}" :key="index">
            <video 
        ...
        <div class="absolute inset-y-0 left-0 flex items-center px-4">
            <button @click="current = current === 0 ? {{ count($videos) - 1 }} : current - 1"
                class="bg-white/60 hover:bg-white text-black p-2 rounded-full shadow">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[48px] h-[48px]" fill="#000000" viewBox="0 0 256 256">
                    <path d="M224,128a8,8,0,0,1-8,8H59.31l58.35,58.34a8,8,0,0,1-11.32,11.32l-72-72a8,8,0,0,1,0-11.32l72-72a8,8,0,0,1,11.32,11.32L59.31,120H216A8,8,0,0,1,224,128Z"/>
                </svg>
            </button>
        </div>
        <div class="absolute inset-y-0 right-0 flex items-center px-4">
            <button @click="current = current === {{ count($videos) - 1 }} ? 0 : current + 1"
                class="bg-white/60 hover:bg-white text-black p-2 rounded-full shadow">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[48px] h-[48px]" fill="#000000" viewBox="0 0 256 256">
    ...
    <div class="overflow-auto flex justify-start md:justify-center gap-2 mt-4">
        <template x-for="(video, index) in {{ Js::from($videos) }}" :key="index">
            <div @click="current = index"
                class="w-[100px] h-[80px] bg-gray-200 flex items-center justify-center cursor-pointer border-2"
